...
const run = async () => {
  ...
  console.info(
    `[poster-variants:recent] Starting dryRun=${options.dryRun} limit=${options.limit}`,
  );

  await mongoose.connect(ENV.MONGO.URI, { maxPoolSize: 10 });

  const docs = await MangaModel.find({ poster: { $exists: true, $ne: "" } })
    .sort({ updatedAt: -1 })
    .limit(options.limit)
    .lean();
  ...
  try {
    for (const doc of docs) {
      ...
      if (options.dryRun) {
        console.info(`[dry-run] Would build variants for ${mangaId}`);
        updated += 1;
        continue;
      }
  ...
  const logFile = path.join(
    logDir,
    `backfill-poster-variants-recent-${new Date().toISOString().replace(/[:.]/g, "-")}.json`,
  );
  fs.writeFileSync(logFile, JSON.stringify({ processedItems }, null, 2));

  console.info("\n[poster-variants:recent] Done");
...
run().catch((err) => {
  console.error("[poster-variants:recent] Fatal error", err);
  process.exit(1);
});
