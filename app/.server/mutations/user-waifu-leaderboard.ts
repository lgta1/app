...
export const checkWaifuToUpdate = async (userId: string, waifuId: string) => {
  // IMPORTANT: Do not gate on UserWaifu (history) because cache can be missing while history exists.
  // Gate on the leaderboard doc itself to make this idempotent and self-healing.
  ...
  // 1) Try to insert this waifu into the cache only if it does NOT already exist.
  const inserted = await UserWaifuLeaderboardModel.findOneAndUpdate(
    {
      userId,
      "waifuCollection.waifuId": { $ne: waifuIdStr },
    },
    {
      $push: {
        waifuCollection: {
  ...
  // 2) If waifu already existed, still keep user snapshot fields fresh.
  if (!inserted) {
    await UserWaifuLeaderboardModel.updateOne(
      { userId },
      {
        $set: {
          userName: user?.name,
