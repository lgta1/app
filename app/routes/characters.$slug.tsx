...
import { CharacterModel, type CharacterType } from "~/database/models/character.model";

export const meta: MetaFunction = ({ data }: any) => {
  if (!data?.entity) {
    return [
      { title: "Nhân vật | Vinahentai" },
      { name: "description", content: "Danh sách nhân vật tại Vinahentai" },
    ];
  }
  return [
    { title: `${data.entity.name} | Vinahentai` },
    { name: "description", content: `Truyện có nhân vật ${data.entity.name}` },
  ];
};
...
export async function loader({ params, request }: LoaderFunctionArgs) {
  ...
  const entity = await CharacterModel.findOne({ slug }).lean();
  if (!entity) throw new Response("Không tìm thấy nhân vật", { status: 404 });

  const query: Record<string, any> = {
    characterSlugs: (entity as any).slug,
  ...
  return (
    <div className="container-page mx-auto px-4 py-6">
      <div className="mb-8 flex flex-col items-start justify-start gap-3.5">
        ...
        <p className="text-txt-primary text-sm leading-tight font-normal">Truyện có nhân vật {(entity as any)?.name}</p>
