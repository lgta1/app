// app/routes/api.characters.create.ts
...
async function ensureUniqueSlug(base: string) {
  ...
  while (await CharacterModel.exists({ slug })) {
    slug = `${base}-${i++}`;
...
export async function action({ request }: ActionFunctionArgs) {
  try {
    ...
    const existing = await CharacterModel.findOne({ name: { $regex: new RegExp(`^${escapeRegExp(name)}$`, "i") } });
    if (existing) return new Response(JSON.stringify({ character: toClient(existing) }), { status: 409, headers: { "content-type": "application/json" } });
    ...
    const created = await CharacterModel.create({ name, slug });
    return new Response(JSON.stringify({ character: toClient(created) }), { status: 201, headers: { "content-type": "application/json" } });
  } catch (err) {
    console.error("[api.characters.create] error:", err);
